port par defaut= 8123
/var/lib/hass

presser 10 sec le bouton on/off du dimmer switch
ne pas hesiter a entrer le numero de serie

HACS = Home Assistant Community Store.

* setup local voice recognition

You need Text to speech and speech to text services. 
Add them through wyoming integration and only then you can go through
the voice setup

sudo -u hass /nix/store/j0d9289d0sg7f1l62vd1g3mfw4268dhl-homeassistant-2024.4.4/bin/hass --script auth  --config '/var/lib/hass'  list

* voice detection

  # --preload-model 'ok_nabu'

* zigbee2mqtt (z2m)

  1. enable mqtt service (mosquitto for instance)

	 Z igbee2MQTT:info  2019-11-09T12:19:56: Successfully interviewed '0x00158d0001dc126a', device has successfully been paired
 
** sonoff SNZB-02 
   Change celsius to fahreneit: double tap reset button https://www.zigbee2mqtt.io/devices/SNZB-02D.html

** tuya  ZG-204ZM pir sensor


pour recharger les piles, suffit de faire glisser le derriere !

** tuya RH3040 PIR (grosse tete)

https://www.zigbee2mqtt.io/devices/RH3040.html

* how to reset advices

** philips play

le plus simple est de se connecter via l'app HUE

https://www.zigbee2mqtt.io/devices/915005733701.html

https://www.philips-hue.com/en-gb/support/article/how-to-factory-reset-philips-hue-lights/000004

* integrations

https://www.home-assistant.io/integrations/yamaha_musiccast/


* hass-cli tips

set -x HASS_TOKEN XXX
set -x HASS_SERVER http://XXX

the (http is important else weird errors !)
 hass-cli service call light.turn_on \
   --arguments entity_id=light.lampe_chevet_fenetre_light

  To discover your light entity IDs:

  hass-cli state list 'light.*'
