...
function set_start_end_timestamps()
    ...
    mp.osd_message(
        string.format('%s - %s', seconds_to_ffmpeg_time(start_timestamp), seconds_to_ffmpeg_time(end_timestamp))
    )
end
...
function replay_anki_card()
    if start_timestamp == nil and end_timestamp == nil then
        local subtitles_fragment, subtitles_fragment_start_time, subtitles_fragment_end_time, subtitles_fragment_prev, subtitles_fragment_next =
...
function convert_into_sentences()
    ...
    for i, sub_text in ipairs(subs) do
        ...
        if #sentences > 0 then
            ...
            if
                JOIN_SUBTITLES_INTO_SENTENCES
                and (sub_start - prev_sub_end) <= 2
...
function export_video_fragment(start_timestamp, end_timestamp)
    ...
    if VIDEO_FORMAT_HTML5 ~= true then
        ...
        args = {
            ...
            '0:v:0',
            '-map',
            string.format('0:a:%d', ff_aid),
            '-af',
            string.format('afade=t=in:curve=ipar:st=%.3f:d=%.3f,afade=t=out:curve=ipar:st=%.3f:d=%.3f', 0, d, t - d, d),
            '-c:v',
            'libx264',
            '-preset',
            'medium',
    ...
    if VIDEO_WIDTH == -1 then
        VIDEO_WIDTH = -2
    end
    if VIDEO_HEIGHT == -1 then
        VIDEO_HEIGHT = -2
    end
...
function export_screenshot()
    ...
    if IMAGE_WITH_SUBTITLES then
        ...
        local args = {
            ...
            '-y',
            '-ss',
            seconds_to_ffmpeg_time(time_pos),
            '-i',
            video_input,
            '-vframes',
            '1',
...
function get_subtitles_fragment(subtitles, subtitles_start, subtitles_end)
    ...
    for i, sub_text in ipairs(subtitles) do
        ...
        if
            (start_timestamp ~= nil and (start_timestamp <= sub_start and sub_end <= end_timestamp))
            or (start_timestamp == nil and (sub_start <= time_pos and time_pos <= sub_end))
        then
    ...
    return table.concat(subtitles_fragment, '<br />'),
        subtitles_fragment_start_time,
        subtitles_fragment_end_time,
        subtitles_fragment_prev,
