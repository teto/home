...
setmetatable(M, {
    __index = function(_, k)
        -- Filter out OpenAI's default models because everyone uses their own ones with LlamaCpp
        if k == 'model' or k == 'model_names' then
            return nil
        end
...
M.api_key_name = '' -- LlamaCpp typically doesn't require API keys for local use
...
-- LlamaCpp is disabled by default. Users should override is_env_set()
            ...
            if #content > 0 then
                ...
                for _, item in ipairs(content) do
                    if type(item) == 'table' and item.type == 'text' then
                        table.insert(text_content, item.text)
                    end
                end
...
---@class avante.LlamaCppFunction
---@field name string
---@field arguments table

---@class avante.LlamaCppToolCall
---@field function avante.LlamaCppFunction

---@param tool_calls avante.LlamaCppToolCall[]
...
function M:parse_curl_args(prompt_opts)
    ...
    if not provider_conf.model or provider_conf.model == '' then
        Utils.error('LlamaCpp: model must be specified in config')
        return nil
    end

    if not provider_conf.endpoint then
        Utils.error('LlamaCpp: endpoint must be specified in config')
        return nil
    end
...
M.on_error = function(result)
    local error_msg = 'LlamaCpp API error'
    if result.body then
    ...
    Utils.error(error_msg, { title = 'LlamaCpp' })
end
...
local function query_models(opts, timeout)
    ...
    if not provider_conf.endpoint then
        return nil, 'LlamaCpp requires endpoint configuration'
    end
    ...
    -- Request the model tags from LlamaCpp
    local response = {}
    local job = curl.get(tags_url, {
        ...
        on_error = function(err)
            response = { exit = err.exit }
        end,
    })
    local job_ok, error = pcall(job.wait, job, timeout or 10000)
    if not job_ok then
        return nil, 'LlamaCpp: curl command invocation failed: ' .. error
    elseif response.exit ~= 0 then
        local err_msg = curl_errors[response.exit] or ('curl returned error: ' .. response.exit)
        return nil, 'LlamaCpp: ' .. err_msg
    elseif response.status ~= 200 then
        return nil, 'Failed to fetch LlamaCpp models: ' .. (response.body or response.status)
    end

    -- Parse the response body
    local ok, res_body = pcall(vim.json.decode, response.body)
    if not ok then
        return nil, 'Failed to parse model list query response'
    end
