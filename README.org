...
Dont expect to be able to use it as it is a spaghetti of hacks, for instance it relies on forks of nixpkgs and home-manager.

The main difficulty to allow reproducible deployments is the handling of secrets.
This is why my nixosConfigurations can be built with and without secrets depending on a boolean `withSecrets` forwarded via `specialArgs`.

My main secrets are handled with sops and not gitted (no way I would put those public).
