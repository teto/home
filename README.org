...
This repository is *tetos*, teto's OS, nixos-based, tailored to my needs.
This repo contains the configurations for my machines/servers.

I make it public because:
- I believe in libre software
- this makes sharing snippets easier
- as well as bootstrapping, no need for github login

Dont expect to be able to use this repo as is: it is a spaghetti of hacks, commented code and relies on forks of nixpkgs and home-manager.
...
1. SERVER: Enable the pixiecore service (`services.pixiecore.enable = true`)
2. NEW_MACHINE: select "Boot via PXE" on boot
3. NEW_MACHINE: echo LUKS_PASSWORD > /tmp/secret.key
...
#TODO gpg need to trust the imported key
...
gpg --export-secret-keys --armor nani.wtf > ~/gpg_keys.pem

# Import
gpg --import ~/gpg_keys.pem
@end-src
