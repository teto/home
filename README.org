...
* nixos install via PXE

We use [[https://github.com/nix-community/nixos-anywhere][nixos-anywhere]] + [[https://github.com/nix-community/disko][disko]], aka we need a SERVER machine in the same LAN as the NEW machine to deploy from. 
1. SERVER: Enable the pixiecore service
2. NEW: start via PXE
3. NEW: echo LUKS_PASSWORD > /tmp/secret.key
4. SERVER: run `just bootstrap`
